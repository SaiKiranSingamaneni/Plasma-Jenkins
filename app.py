...
def check(email):
    url = "https://6jmtfsgu56.execute-api.ap-south-1.amazonaws.com/Login?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = " https://xssuds5ajg.execute-api.ap-south-1.amazonaws.com/Registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://rvr99lueui.execute-api.ap-south-1.amazonaws.com/CountBloodData/countblooddata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    print(address)
    url = "https://ks195ftz89.execute-api.ap-south-1.amazonaws.com/requestBlood/getblooddata?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
